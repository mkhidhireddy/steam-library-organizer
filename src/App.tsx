...
function App() {
  ...
  const suggestedCollections = useMemo(() => ["Warhammer", "Star Wars"].filter((candidate) => games.some((game) => (game.approvedIp || detectIp(game.name)) === candidate) && !collections.some((collection) => collection.name === candidate)), [games, collections]);
  ...
  function addSuggestedCollection(ip: string) { saveCollections([...collections, { name: ip, tag: "", ip }]); }
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      ...
      {view === "collections" && <><header className="page-head"><div><p className="kicker">RULE-DRIVEN</p><h2>Smart Collections</h2></div></header>{suggestedCollections.length > 0 && <section className="suggestions"><h3>Suggested collections</h3>{suggestedCollections.map((candidate) => <article key={candidate}><div><strong>{candidate}</strong><p>{games.filter((game) => (game.approvedIp || detectIp(game.name)) === candidate).length} matching games</p></div><button onClick={() => addSuggestedCollection(candidate)}>Add {candidate} collection</button></article>)}</section>}<section className="builder"><label>Collection name<input value={name} onChange={(event) => setName(event.target.value)} placeholder="Co-op RPGs"/></label><label>Required store tag<select value={tag} onChange={(event) => setTag(event.target.value)}><option value="">Any tag</option>{tags.map((value) => <option key={value}>{value}</option>)}</select></label><label>IP<select value={ip} onChange={(event) => setIp(event.target.value)}><option value="">Any IP</option><option>Warhammer</option><option>Star Wars</option></select></label><button onClick={addCollection}>Save collection</button></section><div className="collection-list">{collections.map((collection) => <article key={collection.name}><div><h3>{collection.name}</h3><p>{[collection.tag, collection.ip].filter(Boolean).join(" + ") || "All games"}</p></div><strong>{matches(collection).length}</strong><span>matches</span><button className="quiet" onClick={() => saveCollections(collections.filter((item) => item.name !== collection.name))}>Delete</button></article>)}</div></>}
